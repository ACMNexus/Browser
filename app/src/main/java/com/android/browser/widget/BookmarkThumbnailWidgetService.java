...
public class BookmarkThumbnailWidgetService extends RemoteViewsService {
    ...
    static void deleteWidgetState(Context context, int widgetId) {
        File file = ReflectUtils.getInstance().getSharedPrefsFile(context, String.format("widgetState-%d", widgetId));
        if (file.exists()) {
            if (!file.delete()) {
                file.deleteOnExit();
            }
    ...
    static void removeOrphanedStates(Context context, int[] widgetIds) {
        File prefsDirectory = ReflectUtils.getInstance().getSharedPrefsFile(context, "null").getParentFile();
