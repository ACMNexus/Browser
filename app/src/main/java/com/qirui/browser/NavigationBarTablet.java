...
public class NavigationBarTablet extends NavigationBarBase implements StateListener {
    ...
    public void onConfigurationChanged(Configuration config) {
        ...
        if (mUrlInput.hasFocus()) {
            if (mHideNavButtons && (mNavButtons.getVisibility() == VISIBLE)) {
                int aw = mNavButtons.getMeasuredWidth();
                mNavButtons.setVisibility(GONE);
                mNavButtons.setAlpha(0f);
                mNavButtons.setTranslationX(-aw);
            } else if (!mHideNavButtons && (mNavButtons.getVisibility() == GONE)) {
                mNavButtons.setVisibility(VISIBLE);
    ...
    private void hideNavButtons() {
        ...
        Animator anim1 = ObjectAnimator.ofFloat(mNavButtons, TRANSLATION_X, 0, -awidth);
    ...
    private void showNavButtons() {
        ...
        if (!mBaseUi.blockFocusAnimations()) {
            int awidth = mNavButtons.getMeasuredWidth();
            Animator anim1 = ObjectAnimator.ofFloat(mNavButtons, TRANSLATION_X, -awidth, 0);
