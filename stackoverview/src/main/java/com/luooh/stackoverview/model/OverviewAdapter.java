...
package com.luooh.stackoverview.model;

import android.content.Context;
import android.view.ViewGroup;
import com.luooh.stackoverview.misc.OverviewConfiguration;
import com.luooh.stackoverview.views.OverviewCard;
...
public abstract class OverviewAdapter<VH extends ViewHolder, Model extends Object> {
    ...
    public void setItems(List<Model> datas) {
        if(datas != null && datas.size() > 0) {
            for (int index = 0; index < datas.size(); index++) {
                notifyDataSetInserted(datas.get(index), mItems.size());
            }
        }
    }
